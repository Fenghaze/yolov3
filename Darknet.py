...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target):
        ...
        anchors = torch.FloatTensor(self.anchors).view(nA, -1).to(self.device)  # [nA, 2]
        cls_anchor_dim = nB * nA * nH * nW  # nB*3*13*13

        output = output.view(nB, nA, (5+nC), nH, nW)    # [n, 3, 25, 13, 13]
        cls_grid = torch.linspace(5, 5 + nC - 1, nC).long().to(self.device)      # 5,6,7...5+nC-1 , [20]
        ix = torch.LongTensor(range(5)).to(self.device)
        pred_boxes = torch.FloatTensor(4, cls_anchor_dim).to(self.device)       # 用于保存每个anchor box的(x,y,w,h)坐标信息, [4, n*3*13*13]

        # 获取每个anchor的坐标
        # index_select(2, ix[0:4])，其中2表示维度，ix[0:4]表示索引，只选择了坐标的四列
        coord = output.index_select(2, ix[0:4]).view(nB * nA, -1, nH * nW).transpose(0, 1) \
            .contiguous().view(-1, cls_anchor_dim)  # 每一个anchor的x, y, w, h, [4, nB*3*13*13]
        coord[0:2] = coord[0:2].sigmoid()  # 前两行表示坐标，后两行表示长宽

        # 获取每个anchor的置信度
        conf = output.index_select(2, ix[4]).view(cls_anchor_dim).sigmoid()     # 置信度,[1, n*3*13*13]

        # 获取每个anchor的class
        cls = output.index_select(2, cls_grid)  # [n, 3, 20, 13, 13]
        cls = cls.view(nB*nA, nC, nH*nW).transpose(1, 2).contiguous().view(cls_anchor_dim, nC).to(self.device) # [n*3*13*13 ,20]

        # 获取网格的(x, y),0~nW-1,0~nH-1
        grid_x = torch.linspace(0, nW - 1, nW).repeat(nB * nA, nH, 1).view(cls_anchor_dim).to(self.device)  # [n*3*13*13]
        grid_y = torch.linspace(0, nH - 1, nH).repeat(nW, 1).t().repeat(nB * nA, 1, 1) \
            .view(cls_anchor_dim).to(self.device)   # [n*3*13*13]

        # 获取anchor信息
        anchor_w = anchors.index_select(1, ix[0]).repeat(nB, nH * nW).view(cls_anchor_dim) # [n*3*13*13]
        anchor_h = anchors.index_select(1, ix[1]).repeat(nB, nH * nW).view(cls_anchor_dim) # [n*3*13*13]
        ...
        pred_boxes = convert2cpu(pred_boxes.transpose(0, 1).contiguous().view(-1, 4)).detach()  # [n*3*13*13, 4]
